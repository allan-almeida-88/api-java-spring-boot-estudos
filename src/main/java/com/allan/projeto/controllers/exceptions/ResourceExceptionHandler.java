...
public class ResourceExceptionHandler {
    ...
    public ResponseEntity<StardartError> resourceNotFound(ResourceNotFoundException e, HttpServletRequest request) {
        ...
        StardartError err = new StardartError(Instant.now(), status.value(), error, e.getMessage(), request.getRequestURI());

        return ResponseEntity.status(status).body(err);
    }

    @ExceptionHandler(DatabaseException.class)
    public ResponseEntity<StardartError> databaseException(DatabaseException e, HttpServletRequest request) {
